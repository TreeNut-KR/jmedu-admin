...
import { GetStudentAttendancesSchema } from "@/schema";

export default async function getStudentAttendances(req: NextApiRequest, res: NextApiResponse) {
  try {
    // 접근 권한 검증
    await checkPermission("http", "students_attendance_view", req, res);

    // 필요한 쿼리
    const { data: params, error: paramsError } = GetStudentAttendancesSchema.safeParse(req.query);
    ...
    // 학생 출결 목록 조회 시작
    adminLog(`학생 출결 목록 조회 (${paramsToString(params)})`, req);
